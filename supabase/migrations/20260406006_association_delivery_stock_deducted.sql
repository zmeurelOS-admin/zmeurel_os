...
comment on column public.comenzi.stock_deducted is
  'Flag tehnic: stocul a fost dedus la livrarea comenzii din magazinul asociației.';

create index if not exists comenzi_association_delivery_idx
  on public.comenzi (data_origin, status, data_livrare)
  where data_origin = 'magazin_asociatie';
