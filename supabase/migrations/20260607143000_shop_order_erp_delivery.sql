-- Bridge B2C shop deliveries into the existing ERP order/sale/stock flow.
-- This migration adds structures only and does not backfill or mutate existing rows.

alter table public.shop_products
  add column if not exists unit_weight_kg numeric;

create table if not exists public.shop_order_erp_links (
  shop_order_id uuid primary key references public.shop_orders(id),
  comanda_id uuid not null unique references public.comenzi(id),
  tenant_id uuid not null references public.tenants(id),
  weight_snapshot jsonb not null default '[]'::jsonb,
  created_at timestamptz not null default now()
);

create index if not exists shop_order_erp_links_tenant_idx
  on public.shop_order_erp_links (tenant_id);

alter table public.shop_order_erp_links enable row level security;

drop policy if exists shop_order_erp_links_tenant_select on public.shop_order_erp_links;
create policy shop_order_erp_links_tenant_select
on public.shop_order_erp_links
for select
to authenticated
using (tenant_id = public.current_tenant_id());

revoke all on table public.shop_order_erp_links from anon;
revoke all on table public.shop_order_erp_links from authenticated;
grant select on table public.shop_order_erp_links to authenticated;
