...
-- Only change from the DB state at time of application: `v_tarif is null or v_tarif <= 0` → `v_tarif is null`.

CREATE OR REPLACE FUNCTION public.create_recoltare_with_stock(p_data date, p_parcela_id uuid, p_culegator_id uuid, p_kg_cal1 numeric DEFAULT 0, p_kg_cal2 numeric DEFAULT 0, p_observatii text DEFAULT NULL::text, p_tenant_id uuid DEFAULT NULL::uuid)
 RETURNS recoltari
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.update_recoltare_with_stock(p_recoltare_id uuid, p_data date, p_parcela_id uuid, p_culegator_id uuid, p_kg_cal1 numeric DEFAULT 0, p_kg_cal2 numeric DEFAULT 0, p_observatii text DEFAULT NULL::text)
 RETURNS recoltari
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
