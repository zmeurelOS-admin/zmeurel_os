...
begin
  ...
  if not (public.is_tenant_owner(v_tenant_id) or public.operator_can_write('tratamente')) then
...
revoke all on function public.create_recoltare_with_stock(date, uuid, uuid, numeric, numeric, text, uuid) from public;
revoke all on function public.update_recoltare_with_stock(uuid, date, uuid, uuid, numeric, numeric, text) from public;
revoke all on function public.delete_recoltare_with_stock(uuid) from public;
...
revoke all on function public.upsert_with_idempotency(text, jsonb) from public;

grant execute on function public.create_recoltare_with_stock(date, uuid, uuid, numeric, numeric, text, uuid) to authenticated, service_role;
...
grant execute on function public.delete_recoltare_with_stock(uuid) to authenticated, service_role;
...
grant execute on function public.upsert_with_idempotency(text, jsonb) to authenticated, service_role;
