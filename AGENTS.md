...
- `/livrari` citește doar comenzile `shop_orders.status = in_livrare` din ziua curentă București: `delivery_date` când este setată, cu fallback pe data locală a `created_at` pentru rândurile legacy. Ruta zilei este ordonată după `delivery_position NULLS LAST`, apoi `created_at`; reorder-ul se persistă tenant-aware și tranzacțional prin `POST /api/shop/b2c/orders/reorder` + RPC `reorder_shop_deliveries_today`, care setează pozițiile și completează `delivery_date` doar unde lipsește. Un query separat, tenant-scoped, alimentează bannerul read-only pentru comenzile programate azi cu status `noua`/`confirmata`, fără să schimbe lista sau RPC-ul rutei active.
