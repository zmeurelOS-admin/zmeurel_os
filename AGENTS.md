...
- **Topologie staging separată (ops)**: există acum un proiect Supabase dedicat `zmeurelOS-staging` (`qinpqsqeaagjfobqwfwx`), separat de `zmeurelOS-dev` și `zmeurelOS-prod`; mapping-ul Vercel non-prod se face prin env-uri branch-specific pe `preview` pentru branch-ul `staging`. Checkerul read-only `scripts/check-staging-readiness.mjs` / `npm run check:staging-readiness` validează această topologie și semnalează gap-urile rămase (ex. `SITE_URL`, `supabase/config.toml`, preview deployment efectiv pentru branch-ul `staging`).
