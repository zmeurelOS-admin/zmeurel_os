...
- Campania UI „Zmeură 2026” pentru `comanda.zmeurel.ro` este compusă în `src/app/comanda/ShopClient.tsx` din secțiunile dedicate din `src/app/comanda/components/`. `GET /api/shop/campaign/[slug]` citește prin clientul Supabase anon campania activă și milestone-urile, cu cache de 30 secunde; `src/lib/shop/campaign-mock.ts` rămâne fallback tipat pentru loading/eroare și păstrează conținutul editorial static (reguli + leaderboard).
