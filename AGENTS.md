...
- **Vânzări** (`src/app/(dashboard)/vanzari/VanzariPageClient.tsx`): dialoguri Add/Edit — `desktopFormWide`, `FormDialogSection`, `DesktopFormGrid` + `VanzareFormSummary` pe `md+` (total `cantitate × preț`, status, contor vânzări pe client din lista deja în cache, prop `tenantVanzari`); mobilul rămâne fără aside.
