...
- **Livrări ERP (`/livrari`)**: livrarea totală a `shop_orders` rămâne fluxul existent prin `PATCH status=livrata`, iar dialogul de confirmare suportă acum și livrare parțială pentru ambele surse ERP (`shop_orders` și `comenzi` manuale). Pentru shop se folosește RPC-ul `deliver_shop_order_atomic_partial`, iar pentru comenzi manuale se folosește `deliverComanda()` / `deliver_order_atomic`; cantitatea totală disponibilă în UI este derivată în `src/lib/livrari/types.ts` (`cantitate_kg` din `items` pentru shop, respectiv `comenzi.cantitate_kg` pentru manuale).
- **Stoc vandabil cal1 + rezervări**: din `2026-06-22`, recoltările noi scriu în `miscari_stoc` doar intrarea `cal1` prin `sync_recoltare_stock_movements`; `cal2` rămâne în `recoltari`, dar nu mai intră în stocul comercial disponibil. Punctul unic de adevăr pentru disponibilitate este `public.get_sellable_cal1_stock_summary()`. Trecerea în `in_livrare` pentru comenzi manuale și `shop_orders` folosește `public.stock_reservations` prin RPC-urile `set_comanda_in_delivery_with_reservation` / `set_shop_order_in_delivery_with_reservation`; ieșirea din `in_livrare` eliberează rezervarea, iar `deliver_order_atomic` consumă split-ul rezervat dacă există, păstrând fallback-ul legacy pentru comenzile istorice fără rezervare.
...
- Marcarea unei comenzi B2C ca livrată folosește exclusiv RPC-ul `deliver_shop_order_atomic`: validează `shop_products.unit_weight_kg`, creează o comandă ERP agregată în kg, apelează neschimbat `deliver_order_atomic` pentru vânzare + stoc și persistă legătura idempotentă în `shop_order_erp_links`, inclusiv snapshot-ul greutăților folosite. După livrare, RPC-ul validează orice `shop_campaign_milestone_rewards.status = pending` legat de comandă; aceeași reconciliere rulează și pe retry-ul `already_delivered`, fără efect pentru comenzile fără reward. Endpointul nu trebuie să revină la un simplu update de status.
...
- Checkout-ul public rămâne izolat de ERP; legătura cu `comenzi` și stoc se creează numai la confirmarea livrării, prin bridge-ul atomic documentat mai sus.
