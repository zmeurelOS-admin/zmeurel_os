...
- Campania UI „Zmeură 2026” pentru `comanda.zmeurel.ro` este compusă în `src/app/comanda/ShopClient.tsx` din secțiunile dedicate din `src/app/comanda/components/`; datele temporare sunt centralizate și tipate în `src/lib/shop/campaign-mock.ts`. Meter-ul, pragurile, clasamentul și detectarea locală a pragului sunt doar prezentare/mock și nu modifică payload-ul, schema sau statusurile `shop_orders`.
- Backend-ul campaniilor sezoniere folosește `shop_campaigns`, `shop_campaign_milestones`, `shop_campaign_adjustments` și `shop_campaign_milestone_rewards`; campaniile aparțin unui tenant prin FK la `public.tenants(id)`, iar comenzile pot fi clasificate prin `shop_orders.order_kind` și asociate opțional prin `shop_orders.campaign_id`. Rolurile publice au doar accesul de citire permis de RLS; mutațiile campaniei sunt rezervate RPC-urilor `SECURITY DEFINER` din faza următoare.
