...
- Monitoring: console error logging + Vercel Analytics/Speed Insights.
...
- PWA/installability: manifestul canonic este `src/app/manifest.ts`, iar assetele service worker importate din `public/` (`/sw.js`, `/push-handlers.js`, `/workbox-*.js`, iconurile PWA) trebuie să rămână publice în `src/proxy.ts`; dacă adaugi importuri noi în SW, actualizează allowlist-ul din proxy înainte de deploy.
- **Strategie PWA actualizată în 2026-05-08**: precache minim de install (`/offline` + iconuri/manifest critice), runtime caching pe categorii (`pages-cache`, `next-static`, `styles`, `next-image`, `images`, `fonts`, `api-cache`, `supabase-rest`) și fallback document către `/offline`; update-urile SW rămân non-intruzive (`skipWaiting: false`, `clientsClaim: false`) și sunt activate din UI prin toast de refresh.
...
- **Google Contacts (cron pull + sync manual + push manual)**: `GET /api/cron/sync-google-contacts`, autentificat exclusiv cu `Authorization: Bearer ${CRON_SECRET}`, rulează zilnic la `06:00 UTC`. Configurarea activă și `sync_token` sunt citite din `integrations_google_contacts`, iar `refresh_token` poate fi criptat cu `GOOGLE_TOKENS_ENCRYPTION_KEY`; `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` și `CRON_SECRET` rămân server-only. Sincronizarea incrementală folosește Google People API, face fallback imediat la full sync pentru token expirat (`410`) și scrie în `clienti` prin `service_role`, în batch-uri de 50, folosind `google_resource_name` ca identitate. În `/clienti`, starea sanitizată `sync_enabled` / `last_sync_at` este încărcată server-side; butonul manual apelează `POST /api/integrations/google/sync-now`, care validează same-origin, sesiunea și tenantul operațional înainte să invoce cronul cu secretul server-side și să recitească `last_sync_at`. Ruta `sync-now` este exceptată exact din redirectul auth al proxy-ului pentru a putea returna 401 JSON, dar aplică autentificarea completă în handler. Crearea și editarea manuală a unui client declanșează separat best-effort `POST /api/integrations/google/push-client`; importul CSV nu declanșează push. Scriptul local `node scripts/google-oauth-init.mjs` cere scope-ul write `contacts` (care include read); este necesară reautorizarea după trecerea de la `contacts.readonly`.
...
- **Regiune deploy (latență middleware)**: proiectul Supabase LIVE e în `eu-north-1` (Stockholm). `vercel.json` fixează `"regions": ["arn1"]` (Stockholm) ca funcțiile să fie co-locate cu DB-ul; `/api/chat` are `preferredRegion = 'arn1'` aliniat. La schimbarea regiunii Supabase, actualizează ambele. Fiecare request protejat face `supabase.auth.getUser()` + rezolvare tenant, deci co-locarea taie latența round-trip.
- **Rezoluție operator în proxy (un singur query `farm_members`/request)**: proxy-ul interoghează `farm_members` (service-role) o singură dată, înaintea rezolvării de tenant. Dacă user-ul e operator, tenant-ul e luat direct din acel rând, fără a mai apela `getTenantIdByUserIdOrNull()` (care ar reinteroga `farm_members`). RLS `member can read own row` (`user_id = auth.uid()`) garantează același rezultat ca via clientul de sesiune. Pentru calea owner/profil, când query-ul admin a reușit cu miss (user-ul nu e operator), proxy folosește `getNonOperatorTenantIdOrNull()` din `src/lib/tenant/get-tenant.ts` (owned → `profiles.tenant_id`, sare peste sub-query-ul de operator) — deci și owner-ul face un singur query `farm_members`/request. `getTenantIdByUserIdOrNull()` rămâne neschimbată și e folosită ca fallback DOAR când query-ul admin a eșuat (nu doar miss), ca să re-verifice operator. Nu folosi `getNonOperatorTenantIdOrNull()` în afara proxy-ului: pentru un operator ar rezolva greșit la tenant-ul owned/profil.
- **Operatori fermă / acces pe module**: operatorii sunt membri în `farm_members` cu `role = operator` și `modules_access` JSON; nu primesc tenant propriu prin invite și `profiles.tenant_id` rămâne `NULL` pentru conturile create din `/invite/[token]`. Effective tenant este rezolvat owner-first (`tenants.owner_user_id` / profil existent), apoi operator activ prin `farm_members.tenant_id` (`public.current_tenant_id()` și `src/proxy.ts`). Proxy-ul face allowlist pe rute din `src/lib/farm-members/access.ts`, blochează owner-only (`/settings`, `/admin`, `/asociatie`, `/planuri` etc.) și injectează header-ele `x-zmeurel-member-role`, `x-zmeurel-access-module`, `x-zmeurel-access-level` pentru UI read/write. Enforcement-ul real pentru write/delete este în DB: RLS + RPC guards folosesc helperii SQL `is_tenant_owner`, `operator_can_write`, `operator_is_operator`; owner-ul are bypass total, operatorul scrie doar module cu `level = write` și nu șterge niciodată. Ține manual aliniate `src/lib/farm-members/access.ts` și helperii SQL (module, niveluri, fallback legacy `comenzi` + `livrari` write). RPC-uri cu guard de operator: `create_recoltare_with_stock` 7-arg, `update_recoltare_with_stock`, `delete_recoltare_with_stock`, `set_comanda_in_delivery`, `set_comanda_delivered`, `mark_comanda_incasata`, `promote_shop_order_to_comanda`, `set_shop_order_in_delivery`, `set_shop_order_delivered`, `reorder_shop_deliveries_today`, `delete_comanda_atomic`, `reopen_comanda_atomic`, `upsert_plan_tratament_cu_linii`, `upsert_with_idempotency`. Flow-ul livrator cu link tokenizat rămâne separat.
...
- **Logout sesiune (server)**: `POST /api/auth/sign-out` — `validateSameOriginMutation`, `createClient().auth.signOut()`, `303`; implicit la `/`, opțional `?next=/path` (path relativ, ex. `?next=/login` după ștergere cont). UI: `LogoutButton`, `MoreMenuDrawer`, `MoreMenu`, `DemoBanner` și formularul „Creează-ți ferma” / ștergere cont din setări folosesc același POST. Toți callerii așteaptă `await prepareClientBeforeServerSignOut(queryClient, { unsubscribePush })` înainte de `form.submit()` — funcția curăță subscripția push a device-ului curent (best-effort, erorile sunt înghițite ca să nu blocheze logout-ul), resetează singleton-ul Supabase și golește React Query.
...
- **Dashboard shell lazy overlays**: componentele non-critice din shell-ul dashboard (`PushPermissionBanner`, `BetaWidget`, `AiPanel`) se montează prin `src/components/layout/DashboardShellSlots.tsx` cu `next/dynamic` + `ssr: false`; `BottomTabBar`, `UnifiedMobileFab` și `DashboardSidebarLoader` rămân eager.
- **Dashboard widget grid (desktop)**: view normal pe `/dashboard` (`md+`) randează widget-urile cu `DashboardStaticWidgetGrid` (CSS grid + `compactLayoutVertical` din `src/lib/dashboard/grid-compact.ts`, paritate cu `compactType="vertical"` RGL); `react-grid-layout/legacy` se încarcă lazy doar la `editMode` prin `DashboardEditableGrid` (`next/dynamic`, `ssr: false`). Mobil (`md:hidden`) rămâne stack vertical, fără RGL.
...
- **Comenzi + Livrări ERP (`/comenzi`, `/livrari`)**: `public.comenzi` este sursa unică de citire pentru dashboard, inclusiv bridge-urile Shop (`data_origin = shop_order_bridge`). În `/comenzi`, comenzile cu status deschis se împart exhaustiv: **Active** = `data_livrare IS NULL`, **Programate** = `data_livrare IS NOT NULL`; cardul operațional compară strict `recoltari.kg_cal1` de azi cu necesarul comenzilor deschise de azi/rest. Badge-ul „Shop” se derivează din `data_origin`, însă toate tranzițiile dashboard (programare, intrare în livrare, livrare, anulare) rulează pe rândul `comenzi`; livrarea totală/parțială folosește `set_comanda_delivered` prin `deliverComanda()`. `shop_orders` rămâne doar intake/checkout public și snapshot de compatibilitate: triggerul `sync_shop_order_status_from_comanda` îi oglindește statusul bridge-ului. RPC-urile `sync_shop_order_bridge_status`, `set_shop_order_in_delivery` și `set_shop_order_delivered` rămân pentru endpointul public B2C, nu pentru listele dashboard. La livrare se alege `platit` sau `neplatit`; starea și `data_incasare` rămân exclusiv în `vanzari`, iar `mark_comanda_incasata` este idempotent. Fluxul nu scrie în `miscari_stoc` sau `stock_reservations`.
- **Stoc vandabil cal1 derivat**: din `2026-06-26`, punctul unic de adevăr pentru disponibilitate este `public.get_sellable_cal1_stock_summary()`, calculat derivat din `recoltari.kg_cal1`, `comenzi` livrate/în livrare, `ajustari_stoc.delta_kg` și `shop_orders` istorice nepromovate. `miscari_stoc` și `stock_reservations` rămân arhive/read-only pentru fluxul de comenzi; RPC-urile vechi rămân definite pentru rollback, dar frontendul nu le mai apelează. Pagina `/stocuri` afișează doar pool-ul total „Zmeură cal1”, fără stoc per parcelă.
...
- **Comenzi** (`src/app/(dashboard)/comenzi/ComenziPageClient.tsx`) este modulul de referință pentru master–detail desktop (căutare în toolbar, `skipDesktopDataFilter` + `hideDesktopSearchRow` pe listă când filtrarea e în page). Dialogul Add/Edit comandă folosește `desktopFormWide` + `showCloseButton`, secțiuni `FormDialogSection` grupate cu `DesktopFormPanel`, `DesktopFormGrid` și `ComandaFormSummary` doar pe `md+`; datele din panou sunt derivate live din formular (fără query nou), iar footer-ul fix este standardizat prin `DialogFormActions`. Fluxul mobil **Comenzi din mesaj** (`ComenziDinMesajSheet`) face acum parse + editare + creare direct în același sheet, fără PII în URL pe calea normală; fallback-ul spre formularul mare rămâne doar pentru cazuri ambigue (ex. telefon asociat la mai mulți clienți). Comenzile cu `data_origin = magazin_public` afișează badge „Magazin”, filtre origine (Toate / Din magazin / Manuale), grupare euristică în listă (`src/lib/comenzi/magazin-groups.ts`: telefon + zi + origine) cu sortare și chenar stânga în tabel; inspector + `ViewComandaDialog` rezumă liniile aceluiași checkout.
  - **Tipuri comandă manuală**: `public.comenzi.order_kind` suportă `manual`, `cadou`, `consum_propriu`; pentru `cadou` și `consum_propriu`, prețul rămâne `0`, badge-ul dedicat apare în `UnifiedOrderCard`, iar schimbarea statusului din card nu mai deschide WhatsApp. Fluxurile `ComenziDinMesajSheet` și `AiBottomSheet` rămân pe regula veche de preț `> 0`.
- **Recoltări** (`src/app/(dashboard)/recoltari/RecoltariPageClient.tsx`) confirmă același pattern (toolbar + split + inspector); totaluri compacte în toolbar pe desktop, fără `StickyActionBar` pe `md+`. Dialoguri Add/Edit: `desktopFormWide` + `showCloseButton`, secțiuni `FormDialogSection` grupate cu `DesktopFormPanel`, `DesktopFormGrid` și `RecoltareFormSummary` doar pe `md+`; modalul desktop stă la ~70–72rem cu aside sticky și footer fix, iar calculele de kg/calități/tarif/plată rămân în dialog. Mobilul păstrează cardul „Rezumat plată” în flux.
- **Parcele** (`src/components/parcele/ParcelePageClient.tsx`): dialogurile active **Add/Edit teren** (`AddParcelDrawer`, `EditParcelDialog`) refolosesc același pattern desktop ca Recoltări: `desktopFormWide` + `showCloseButton`, `FormDialogSection`, `DesktopFormPanel`, `DesktopFormGrid` și `ParcelFormSummary` doar pe `md+`. `ParcelForm` rămâne componenta unică a câmpurilor; pe desktop câmpurile sunt grupate în „Detalii teren / Locație / Vizibilitate și raportare”, iar pe mobil aside-ul nu apare.
- **Culegători** (`src/app/(dashboard)/culegatori/CulegatorPageClient.tsx`): același split + inspector pe desktop, tabel păstrat; detalii pe mobil rămân în `AppDialog` (deschis doar când viewport-ul nu e desktop). Dialoguri Add/Edit: `desktopFormWide`, `FormDialogSection`, `DesktopFormGrid` + `DesktopFormAside` prin `CulegatorFormSummary` doar pe `md+`; edit folosește `workerStats` deja în pagină, fără query nou.
- **Vânzări** (`src/app/(dashboard)/vanzari/VanzariPageClient.tsx`): din 2026-07-11 pagina este **registru read-only** — vânzările se creează exclusiv prin livrarea comenzilor (`set_comanda_delivered` / `set_shop_order_delivered`). Dialogurile Add/Edit și ștergerea au fost eliminate (create/update/delete_vanzare_with_stock scriau în ledgerul vechi `miscari_stoc`); singura acțiune rămasă este toggle-ul de încasare (`setVanzareStatusPlata` — update financiar direct pe `vanzari`, fără efect pe stoc).
- **Produse** (`src/app/(dashboard)/produse/ProdusePageClient.tsx`): catalog fermă — tabel + inspector desktop (`DesktopSplitPane`), toolbar cu filtre; Add/Edit produse folosesc `desktopFormWide`, `FormDialogSection`, `DesktopFormGrid` + `ProdusFormSummary` pe `md+`; logica de fotografii rămâne în dialog și nu există query suplimentar pentru „vânzări per produs” până când `vanzari`/`comenzi` expun `produs_id` în cache-ul paginii.
- **Cheltuieli** (`src/app/(dashboard)/cheltuieli/CheltuialaPageClient.tsx`): split + inspector pe desktop, lățime `md:max-w-7xl`, totaluri filtru în `DesktopToolbar`; mobil: carduri expandabile neschimbate ca flux. **Formulare Add/Edit**: `AddCheltuialaDialog` / `EditCheltuialaDialog` folosesc `desktopFormWide` pe `AppDrawer`/`AppDialog`, `FormDialogSection` și `DesktopFormGrid` + `DesktopFormAside` pentru rezumat live doar pe `md+`; sub `md` flow-ul rămâne compact.
- **Investiții** (`src/app/(dashboard)/investitii/InvestitiiPageClient.tsx`): același pattern desktop ca OPEX (toolbar cu total în filtru, split, inspector CAPEX); **formulare Add/Edit**: `desktopFormWide`, `FormDialogSection`, `DesktopFormGrid` + `DesktopFormAside` pentru rezumat live doar pe `md+` în `AddInvestitieDialog` / `EditInvestitieDialog`, aliniat cu Cheltuieli; mobil neschimbat ca flux.
- **Stocuri** (`src/app/(dashboard)/stocuri/StocuriPageClient.tsx`): pagină simplă cu pool-ul derivat `Zmeură cal. I` (sursă unică: RPC `get_sellable_cal1_stock_summary`), plus **Ajustări stoc** (din 2026-07-11): buton „Ajustează stocul" → `src/components/stocuri/AjustareStocDialog.tsx` (tip congelat/procesat/pierdere/consum_propriu/corecții/altul, cantitate, dată, motiv) prin RPC `create_ajustare_stoc` (gardă anti-negativ pe disponibilul derivat; INSERT-ul direct pe `ajustari_stoc` e revocat prin RLS) și secțiune „Istoric ajustări" din `src/lib/supabase/queries/ajustari-stoc.ts`.
- **Parcele** (sursă canonică: `src/components/parcele/ParcelePageClient.tsx`): desktop `md+` folosește split workspace cu listă stânga (căutare + selecție clar evidențiată) și inspector dreapta (header + acțiuni principale + taburi `Prezentare` / `Jurnal` / `Tratamente` / `Microclimat`); mobil păstrează lista actuală expandabilă, iar secțiunea expandată afișează tot `Microclimat` cu date reale OpenWeather (variantă compactă, plus link „Vezi microclimat manual" pentru solarii cu loguri). Componenta de afișare `src/components/parcele/MicroclimatAutoCard.tsx` este DRY — singura sursă UI pentru date automate meteo, folosită atât în inspector-ul desktop, cât și în cardul mobil. Selecția pentru meteo se face prin `activeMeteoParcelaId = expandedId ?? desktopSelectedParcelaId ?? filteredParcele[0]?.id`, iar `useMeteo` se apelează o singură dată în parent (zero risc de fetch dublu). `src/app/(dashboard)/parcele/ParcelaPageClient.tsx` rămâne fallback legacy marcat `@deprecated` pentru a evita confuzia între arbori.
...
- **Admin analytics** (`/admin/analytics`): dashboard desktop-first; încărcare metrici în `src/lib/admin/analytics-dashboard-data.ts`, UI în `src/components/admin/analytics/`; filtre URL: `period` (`7d`|`30d`|`90d`), `demo` (`all`|`exclude_demo`|`demo_only`), plus filtre AI (`aiRange`, `aiFlow`, `aiDecisionMode`). Evenimentele AI (`ai_chat_decision`) se persistă în `analytics_events.event_data`. Din `2026-05-08`, secțiunea Tech Health bazată pe Sentry a fost eliminată; fallback-urile UI rămân, iar raportarea erorilor merge doar în `console.error` cu redactare prin `src/lib/logging/redaction.ts`.
...
- Dashboard 2.0 logic is centralized in `src/lib/dashboard/engine.ts`, while tratamentele V2 pentru dashboard folosesc builderul determinist `src/lib/dashboard/treatment-suggestions.ts` + endpointul autenticat `GET /api/dashboard/next-treatment-suggestion`; semnalul nou combină `aplicari_tratament`, `InterventieRelevantaV2`, recurența calculată, PHI, pauza minimă dintre aplicări și ferestrele meteo din `MeteoZi`, fără generare liberă de text.
- În dashboard, cardul „Următorul tratament recomandat” este sursa principală pentru tratamente; `buildDashboardTasks`, `buildDashboardAlerts` și atenționările de parcelă folosesc semnalul V2 când există și păstrează logica din `activitati_agricole` / `culturi.interval_tratament_zile` doar ca fallback pentru ferme fără Tratamente V2 configurat.
- `buildDashboardRecommendations` nu injectează o intrare separată `rec-next-treatment` (acel conținut stă doar în `DashboardNextTreatmentCard`); iar `buildAttentionNowItems` primește `hasNextTreatmentSuggestionCard` ca să evite dublura „Atenție azi” față de același semnal V2.
- Recurența în `Protecție & Nutriție` este doar semnal operațional până la confirmarea utilizatorului: `InterventieRelevantaV2` calculează `urmatoarea_data_estimata`, dar nu auto-generează serii întregi de aplicări. Pregătirea unei repetări se face explicit prin `createAplicarePlanificataDinInterventie` / CTA „Pregătește aplicare”, cu respectarea `numar_repetari_max` și a intervalului mai restrictiv dintre `interval_repetare_zile` și `produse_fitosanitare.interval_min_aplicari_zile`.
...
- Server Components, route handlers, server actions, and server helpers must not runtime-import browser Supabase helpers; `npm run check:server-supabase-boundaries` enforces this by tracing server-side imports to `src/lib/supabase/client.ts`.
...
- Sentry a fost eliminat în `2026-05-08`. Plan de reinstalare: la `50+` utilizatori activi, errors-only, fără Replay.
...
- Modulul `Protecție & Nutriție` are fundație DB paralelă cu `activitati_agricole`, `culture_stage_logs` și `etape_cultura`. Tabelele noi sunt `produse_fitosanitare`, `planuri_tratament`, `planuri_tratament_linii`, `planuri_tratament_linie_produse`, `parcele_planuri`, `stadii_fenologice_parcela`, `aplicari_tratament`, `aplicari_tratament_produse`; nu se rescriu flow-urile existente și nu se mută date din structurile vechi.
...
- Intervențiile manuale/ad-hoc folosesc același editor V2 de produse ca `MarkAplicataSheet`; sursa se salvează ca `manuala`, iar intrările de creare sunt expuse prin add-action în pagina parcelei și în hub-ul global.
- Schema V2 pentru tratamente păstrează `planuri_tratament_linii` fizic, dar semantic linia este acum o intervenție planificată; produsul/doza legacy de pe linie rămân fallback temporar, iar compoziția V2 stă în `planuri_tratament_linie_produse` (1..N produse per intervenție, cu snapshot). Liniile au metadata `sursa_linie = din_plan | adaugata_manual` și `motiv_adaugare` pentru intervenții adăugate ulterior de fermier. Din detaliul planului (`/tratamente/planuri/[planId]`), fermierul poate adăuga intervenții manuale noi în aceeași listă, cu motiv obligatoriu și produse selectate din bibliotecă sau introduse liber.
- Din mai 2026, produsele din intervenții/aplicări pot păstra și `cantitate_text` (text liber de tip „60 ml la 15 l apă”) în `aplicari_tratament_produse` și `planuri_tratament_linie_produse`; UI-ul de „intervenție manuală” folosește acum input text liber pentru cantitate, iar dozele numerice `doza_ml_per_hl` / `doza_l_per_ha` rămân doar pentru compatibilitate istorică.
- După salvarea reușită a unei intervenții manuale, produsele introduse doar prin `nume manual` se auto-sincronizează best-effort în biblioteca tenantului (`produse_fitosanitare`) prin deduplicare fuzzy (Fuse threshold `0.3`) și confirmare toast „asociază vs creează nou”; eșecurile acestui pas nu blochează salvarea intervenției.
- `aplicari_tratament` rămâne execuția operațională și are `sursa = din_plan | manuala`; produsul/doza legacy de pe aplicare rămân fallback temporar, iar produsele efectiv folosite sunt în `aplicari_tratament_produse`, cu snapshot pentru istoric/conformitate. Query/UI/import pot folosi dual-read temporar până sunt refactorizate complet pe V2.
- Data layer-ul din `src/lib/supabase/queries/tratamente.ts` expune modele V2 normalizate (`PlanTratamentV2`, `InterventiePlanV2`, `InterventieProdusV2`, `AplicareTratamentV2`, `AplicareProdusV2`) și face dual-read: citește produsele copil V2 când există, altfel hidratează fallback din câmpurile legacy. Salvarea planurilor prin `upsert_plan_tratament_cu_linii(...)` scrie și `planuri_tratament_linie_produse`, dar RPC-ul păstrează temporar pattern-ul vechi delete+insert pentru linii.
- UI-ul minim V2 pentru planuri editează intervenții cu `produse[]` în `PlanWizardStepLinii` și `LinieEditDialog`; fiecare produs poate veni din biblioteca fermei sau poate fi manual. Câmpurile legacy de pe linie sunt derivate din primul produs doar ca fallback, nu ca sursă principală de editare.
- Execuția unei aplicări din plan folosește `MarkAplicataSheet` cu produse efective V2: formularul pornește din `aplicari_tratament_produse` când există, altfel din `planuri_tratament_linie_produse`, permite override minim și salvează `aplicari_tratament_produse` + `diferente_fata_de_plan`; câmpurile legacy de pe aplicare sunt actualizate din primul produs pentru compatibilitate temporară.
- Relevanța operațională V2 este calculată în data layer (`listInterventiiRelevanteParcela`, `listInterventiiRelevanteHub`) din stadiile observate curente și planurile active; statusurile sunt simple (`de_facut_azi`, `urmeaza`, `intarziata`, `completata_pentru_moment`, `neaplicabila_fara_stadiu`). Doar aplicările `din_plan` cu `plan_linie_id` intră în calculul repetărilor; intervențiile manuale rămân istoric separat și nu acoperă automat o linie din plan.
...
- Import XLSX via `exceljs` + `fuse.js`, flux separat de wizard la `/tratamente/planuri/import`, parser server-side în `POST /api/tratamente/import/parse`, template download în `GET /api/tratamente/template-download`, RPC reutilizat `upsert_plan_tratament_cu_linii(...)`. Fluxul de import începe acum cu un pas de configurare (parcelă + an), iar salvarea trimite `parceleIds` spre RPC pentru a crea direct asocierea în `parcele_planuri` fără suprascrierea planurilor existente. Template-ul oficial este V2 cu foile `Interventii` și `Produse interventii`; `interventie_key` leagă 1..N produse la aceeași intervenție. Parserul păstrează temporar fallback legacy V1 doar pentru fișiere vechi.
- Lista `/tratamente/planuri` nu mai folosește `DesktopSplitPane` sau inspector lateral; este un grid unificat de carduri pe mobil și desktop, iar cardurile citesc agregări din `listPlanuriTratamentComplet` (`linii_count`, `nr_produse`, `tipuri_interventie`, `nr_aplicate`, `parcele_asociate`).
...
- În `ParcelePageClient` (cardurile de culturi din solar), stadiul curent este citit prin `resolveStadiuFenologicCurentParcela` (`src/lib/tratamente/fenofaza-curenta-parcela.ts`) — ultima înregistrare validă, cu afișare per cohortă pe Rubus mixt via `ParcelaStadiuCurentDisplay`; `etape_cultura` / `culturi.stadiu` rămân fallback vizual pentru istoric vechi.
- **Sursă unică UI stadiu curent (parcele)**: `resolveStadiuFenologicCurentParcela` + `resolveStadiiFenologiceCurenteParcela` + `isParcelaRubusMixtFenologie` în `fenofaza-curenta-parcela.ts`; afișare read-only în `ParcelaStadiuCurentDisplay.tsx`, chip-uri editabile în `ParcelaRezumatAgronomicChips.tsx`. **OUT OF SCOPE (de aliniat separat)**: `getStadiuCurentParcela` și `latestStadiuByDate` din `tratamente.ts` (semantici server-side diferite).
...
- Google Contacts cron pull, authenticated manual-client push, and encrypted OAuth tokens (`GOOGLE_TOKENS_ENCRYPTION_KEY`, server-only)
...
- Stock pipeline (din 2026-07-11): `miscari_stoc` este **arhivă înghețată** — niciun flux viu nu mai scrie/citește din ea (recoltările nu mai sincronizează mișcări, vânzările directe manuale au fost eliminate). Disponibilul vine exclusiv din `get_sellable_cal1_stock_summary`; comenzile shop se promovează în `comenzi` la checkout (`promote_shop_order_to_comanda`, idempotent, service-role aware), iar tranzițiile de status se propagă pe bridge prin `sync_shop_order_bridge_status` / `resync_shop_order_bridge_qty`. Ajustările de stoc trec doar prin `create_ajustare_stoc`. Vezi `docs/stock-cal1-reservations.md`.
...
- Pentru comenzile `/comanda`, `SHOP_TENANT_ID` (UUID tenant) este obligatoriu în env server-side; endpointul refuză inserarea dacă lipsește, iar browserul nu poate furniza sau suprascrie tenantul.
- `shop_orders` este tenant-aware: `authenticated` poate face doar `SELECT`/`UPDATE` pentru `tenant_id = public.current_tenant_id()`, iar PATCH-ul generic filtrează suplimentar după `id` și tenant chiar dacă folosește service role. Rândurile istorice cu `tenant_id IS NULL` rămân nemodificate și nu sunt vizibile în ERP.
...
- `/livrari` citește exclusiv `public.comenzi`: lista activă filtrează `status = in_livrare`, iar bannerul pentru azi filtrează `data_livrare` + statusurile `noua`/`confirmata`/`programata`. Originile Shop rămân vizibile prin badge derivat din `data_origin`; ordinea manuală de traseu este locală pe device. Nu se face fetch din `shop_orders` pentru UI-ul dashboard.
- La PATCH-ul unei comenzi Shop spre `in_livrare`, endpointul păstrează `delivery_date` existentă sau setează data curentă în `Europe/Bucharest`; același PATCH acceptă editarea/ștergerea explicită a `delivery_date` pentru precomenzi. `/comenzi` grupează precomenzile neanulate în „Neprogramate”, apoi pe date crescătoare, și permite alegerea inline a datei; UI-ul invalidează listele relevante, afișează toast și navighează la `/livrari` doar la schimbarea statusului în `in_livrare`. `fetchShopOrders` și `fetchShopOrdersInLivrare` atașează separat reward-ul activ (`pending`/`validated`) din `shop_campaign_milestone_rewards`, folosit read-only pentru badge-urile din comenzi și livrări.
- Marcarea unei comenzi B2C ca `in_livrare` promovează întâi `shop_orders` în `comenzi` prin bridge-ul idempotent `promote_shop_order_to_comanda`, apoi validează stocul derivat prin `set_comanda_in_delivery`; endpointul oglindește statusul în `shop_orders` prin wrapper-ul `set_shop_order_in_delivery`. Marcarea ca `livrata` folosește `set_shop_order_delivered`, care livrează comanda ERP legată prin `set_comanda_delivered`, creează o singură `vanzare` financiară și nu scrie în `miscari_stoc`/`stock_reservations`. Confirmarea shop rămâne doar `PATCH status='confirmata'`, fără promovare ERP.
- Prețul magazinului B2C pentru `zmeura` este calculat exclusiv prin `src/lib/shop/pricing.ts`: fiecare pereche de caserole de 500 g costă 35 lei, iar caserola impară rămasă costă 18 lei. `POST /api/shop/b2c/order` ignoră totalul și prețul trimise de browser, validează produsul/cantitatea și persistă totalul recalculat server-side.
- Campania UI „Zmeură 2026” pentru `comanda.zmeurel.ro` este compusă în `src/app/comanda/ShopClient.tsx` din secțiunile dedicate din `src/app/comanda/components/`. `GET /api/shop/campaign/[slug]` citește prin clientul Supabase anon campania activă și milestone-urile, iar clasamentul top 10 este citit server-side cu service role din view-ul `campaign_leaderboard`; view-ul grupează precomenzile neanulate după telefon, expune numai o etichetă mascată derivată din prenumele ultimei comenzi (`***` + ultimele 3 litere, sau doar `***` pentru tokenuri de maximum 3 caractere), orașul nullable și totalul de caserole și nu are grant pentru `anon`. Endpoint-ul are cache de 30 secunde, iar `src/lib/shop/campaign-mock.ts` rămâne fallback tipat pentru loading/eroare sau clasament live gol și păstrează regulile editoriale.
- Clasamentul ERP complet al campaniei este la `/comenzi/campanie`, accesibil din pill-ul comun al modulului Comenzi. Server Component-ul citește `GET /api/shop/campaign/[slug]/admin`, endpoint privat la nivel de handler (`auth.getUser` + tenant din profil) deoarece proxy-ul tratează generic `/api/shop/*` ca public; toate query-urile service-role filtrează explicit `tenant_id` și `campaign_id`. Agregările pure din `src/lib/shop/campaign-admin-queries.ts` furnizează clasamentul pe telefon, totalul real al comenzilor active, evoluția zilnică în fusul `Europe/Bucharest`, distribuțiile livrare/ridicare și Suceava/exterior/neclasificat, count-urile pe status dintr-un query separat care include anulările și programul livrărilor pentru azi + următoarele 6 zile, cu „Neprogramate” separat. Progresul milestone-urilor continuă să folosească `shop_campaigns.current_count`; pagina etichetează separat totalul derivat din comenzile neanulate, afișează toate milestone-urile și exportă CSV exclusiv client-side.
- Backend-ul campaniilor sezoniere folosește `shop_campaigns`, `shop_campaign_milestones`, `shop_campaign_adjustments` și `shop_campaign_milestone_rewards`; campaniile aparțin unui tenant prin FK la `public.tenants(id)`, iar comenzile pot fi clasificate prin `shop_orders.order_kind` și asociate opțional prin `shop_orders.campaign_id`. `POST /api/shop/b2c/order` apelează `place_preorder_atomic` doar când primește `campaign_id`; RPC-ul `SECURITY DEFINER`, accesibil doar `service_role`, serializează per campanie cu advisory lock + `FOR UPDATE`, inserează comanda `preorder`, incrementează `current_count` și acordă atomic cel mult primul milestone traversat de comandă. Checkout-ul `comanda.zmeurel.ro` trimite o cheie `p_idempotency_key` stabilă pe sesiunea formularului, `p_in_suceava` doar pentru livrare și data preferată opțională ca `p_preferred_delivery_date`; selectorul public permite de mâine până la 60 de zile și nu schimbă data dacă utilizatorul îl lasă necompletat. Checkout-ul public nu mai impune minim per zonă în UI; localitatea rămâne obligatorie pentru livrare, `deliveryZone` continuă să fie trimis și salvat pentru ERP, iar route-ul reîncearcă același RPC fără `p_in_suceava` doar când întâlnește vechiul mesaj „Comanda minimă pentru livrare…”. Înainte de write, `POST /api/shop/b2c/check-recent-order` apelează server-side `check_recent_shop_order` cu tenantul din `SHOP_TENANT_ID` și fereastră de 10 minute, iar clientul cere confirmare explicită pentru o comandă recentă. Comenzile fără `campaign_id` rămân `standard`, iar UI-ul afișează felicitarea exclusiv din rezultatul RPC.
...
- Checkout-ul public rămâne izolat de ERP; legătura cu `comenzi` se creează numai când comanda shop intră în livrare, prin bridge-ul atomic documentat mai sus.
