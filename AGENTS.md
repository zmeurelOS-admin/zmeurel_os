...
- Marcarea unei comenzi B2C ca livrată folosește exclusiv RPC-ul `deliver_shop_order_atomic`: validează `shop_products.unit_weight_kg`, creează o comandă ERP agregată în kg, apelează neschimbat `deliver_order_atomic` pentru vânzare + stoc și persistă legătura idempotentă în `shop_order_erp_links`, inclusiv snapshot-ul greutăților folosite. Endpointul nu trebuie să revină la un simplu update de status.
