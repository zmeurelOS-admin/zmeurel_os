...
- **Livrări ERP (`/livrari`)**: livrarea totală și parțială merge prin conducta unică `comenzi`. Pentru comenzi manuale, `deliverComanda()` apelează `set_comanda_delivered`; pentru `shop_orders`, API-ul promovează idempotent prin `promote_shop_order_to_comanda`, apoi folosește `set_shop_order_in_delivery` / `set_shop_order_delivered` și oglindește statusul public. La livrare se alege `platit` sau `neplatit`; starea și `data_incasare` rămân exclusiv în `vanzari`, iar `mark_comanda_incasata` este acțiunea idempotentă de încasare. Fluxul nu scrie în `miscari_stoc` sau `stock_reservations`.
...
- `/livrari` combină acum două surse în client: comenzile `shop_orders.status = in_livrare` din ziua curentă București (prin RPC-ul/ruta existentă, cu `delivery_date` când este setată și fallback pe data locală a `created_at` pentru rândurile legacy) plus comenzile ERP manuale din `public.comenzi` cu `status = in_livrare` și `data_origin IS NULL`. Lista activă rămâne ordonată shop-first după `delivery_position NULLS LAST`, apoi `created_at`; comenzile manuale intră read-only în aceeași listă, fără reorder persistat și fără să schimbe RPC-ul shop. Un query separat, tenant-scoped, alimentează bannerul read-only pentru comenzile programate azi cu status `noua`/`confirmata`, fără să schimbe lista sau RPC-ul rutei active.
