...
- Schema V2 pentru tratamente păstrează `planuri_tratament_linii` fizic, dar semantic linia este acum o intervenție planificată; produsul/doza legacy de pe linie rămân fallback temporar, iar compoziția V2 stă în `planuri_tratament_linie_produse` (1..N produse per intervenție, cu snapshot). Liniile au metadata `sursa_linie = din_plan | adaugata_manual` și `motiv_adaugare` pentru intervenții adăugate ulterior de fermier.
