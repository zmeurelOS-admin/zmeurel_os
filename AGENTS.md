...
- **Google Contacts (cron pull + push manual)**: `GET /api/cron/sync-google-contacts`, autentificat exclusiv cu `Authorization: Bearer ${CRON_SECRET}`, rulează zilnic la `06:00 UTC`. Configurarea activă și `sync_token` sunt citite din `integrations_google_contacts`, iar `refresh_token` poate fi criptat cu `GOOGLE_TOKENS_ENCRYPTION_KEY`; `GOOGLE_CLIENT_ID` și `GOOGLE_CLIENT_SECRET` rămân server-only. Sincronizarea incrementală folosește Google People API, face fallback imediat la full sync pentru token expirat (`410`) și scrie în `clienti` prin `service_role`, în batch-uri de 50, folosind `google_resource_name` ca identitate. Crearea și editarea manuală a unui client declanșează best-effort `POST /api/integrations/google/push-client`: browserul nu așteaptă requestul, dar ruta autentificată validează tenantul și așteaptă People API înainte de răspuns; importul CSV nu declanșează push. Scriptul local `node scripts/google-oauth-init.mjs` cere scope-ul write `contacts` (care include read); este necesară reautorizarea după trecerea de la `contacts.readonly`. Celelalte rute legacy `/api/integrations/google/*` rămân dezactivate (`410`).
