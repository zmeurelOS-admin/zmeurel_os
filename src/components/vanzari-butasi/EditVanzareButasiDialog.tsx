...
export function EditVanzareButasiDialog({ vanzare, open, onOpenChange }: EditVanzareButasiDialogProps) {
  ...
  const summaryClientName = selectedClient?.nume_client || comboInput.trim() || 'Client'
  ...
  return (
    <AppDialog
      ...
      description="Actualizezi rapid comanda și vezi imediat rezumatul live înainte de salvare."
      desktopFormWide
      desktopFormCompact
      showCloseButton
        ...
        <DesktopFormGrid
          ...
          aside={
            <VanzareButasiFormSummary
              ...
              sourceParcelName={selectedParcela?.nume_parcela || '—'}
        ...
        >
                ...
                <div className="relative">
                  ...
                  {comboOpen && (
                    <div className="absolute left-0 right-0 top-full z-50 mt-1 max-h-56 overflow-y-auto rounded-xl border border-[var(--border-default)] bg-[var(--surface-card)] shadow-[var(--shadow-elevated)]">
                      <button
                        type="button"
                        className="flex w-full items-center gap-1.5 border-b border-[var(--divider)] px-3 py-2.5 text-left text-sm text-[var(--text-secondary)] hover:bg-[var(--surface-card-muted)]"
                        onMouseDown={(e) => {
                          e.preventDefault()
                      ...
                      {comboFiltered.length === 0 ? (
                        <p className="px-3 py-2 text-sm text-[var(--text-secondary)]">Niciun client găsit</p>
                      ) : (
                      ...
                      const optionToneClass =
                        option === 'livrata'
                          ...
                          : option === 'anulata'
                            ...
                            : option === 'confirmata' || option === 'pregatita'
                      ...
                      return (
                        <Button
                          ...
                          className={cn(
                            ...
                            isActive ? optionToneClass : 'bg-[var(--agri-surface)] text-[var(--agri-text-muted)]',
                          )}
                        >
                          {statusLabels[option]}
                        </Button>
            ...
            <DesktopFormPanel>
              <div className="grid gap-2.5 md:grid-cols-2 md:gap-x-3 md:gap-y-2.5">
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <Input id="edit_vb_adresa" className="agri-control h-11 md:h-10" {...form.register('adresa_livrare')} />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea id="edit_vb_obs" rows={2} className="agri-control min-h-[3.75rem] md:min-h-[4rem]" {...form.register('observatii')} />
                </div>
              </div>
            </DesktopFormPanel>
          </FormDialogSection>
          ...
          <FormDialogSection>
            <DesktopFormPanel className="space-y-2.5">
              ...
              {isProductsReadonly ? (
                <div className="rounded-xl border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-xs font-medium text-[var(--status-danger-text)]">
                    ...
                    <div key={field.id} className="relative rounded-xl border border-[var(--border-default)] bg-[var(--surface-card-muted)]/55 p-3 shadow-[var(--shadow-soft)]">
                      ...
                      <div className="grid gap-2.5 pr-8 md:grid-cols-12 md:gap-x-3 md:gap-y-2.5">
                        <div className="space-y-1.5 md:col-span-4">
                          <Label>Soi</Label>
                          <Input className="agri-control h-11 md:h-10" disabled={isProductsReadonly} {...form.register(`items.${index}.soi`)} />
                        </div>
