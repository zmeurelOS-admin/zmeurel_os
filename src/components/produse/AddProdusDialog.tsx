...
export function AddProdusDialog({ open, onOpenChange, onSuccess }: AddProdusDialogProps) {
  ...
  const handleFileChange = (slot: 1 | 2, file: File | null) => {
    ...
    if (slot === 1) { setPhoto1(file); setPreview1(url) }
    else { setPhoto2(file); setPreview2(url) }
  }

  const clearPhoto = (slot: 1 | 2) => {
    if (slot === 1) { setPhoto1(null); setPreview1(null); if (input1Ref.current) input1Ref.current.value = '' }
    else { setPhoto2(null); setPreview2(null); if (input2Ref.current) input2Ref.current.value = '' }
  }
      ...
      <form className="space-y-0" onSubmit={form.handleSubmit(handleSubmit)}>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <Input id="add_produs_nume" className="agri-control h-12" placeholder="Ex: Zmeură Cal1" {...form.register('nume')} />
            {form.formState.errors.nume ? <p className="text-xs text-red-600">{form.formState.errors.nume.message}</p> : null}
          </div>

          {/* Categorie + Unitate vânzare */}
          <div className="space-y-2">
            <Label htmlFor="add_produs_cat">Categorie</Label>
            <select id="add_produs_cat" className="agri-control h-12 w-full px-3 text-base" {...form.register('categorie')}>
          ...
          <div className="space-y-2">
            <Label htmlFor="add_produs_unit">Unitate vânzare</Label>
            <select id="add_produs_unit" className="agri-control h-12 w-full px-3 text-base" {...form.register('unitate_vanzare')}>
          ...
          {/* Gramaj + Preț */}
          <div className="space-y-2">
            <Label htmlFor="add_produs_gramaj">Gramaj / unitate (g)</Label>
            <Input
          ...
          <div className="space-y-2 md:col-span-2">
            <Label htmlFor="add_produs_desc">Descriere</Label>
            <Textarea id="add_produs_desc" rows={2} className="agri-control w-full px-3 py-2 text-base" placeholder="Detalii suplimentare..." {...form.register('descriere')} />
          </div>
