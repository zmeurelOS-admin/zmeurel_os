...
describe('generator helpers', () => {
  describe('matchLiniiCuStadii', () => {
    it('potrivește doar liniile pentru stadiile deja atinse', () => {
      ...
      expect(rezultate.map((item) => item.id)).toEqual(['l1', 'l2'])
    })

    it('filtrează doar stadiul cerut când stadiuFiltru este setat', () => {
      const rezultate = matchLiniiCuStadii(linii, stadii, 'inflorit')
    ...
    it('aplică offset-ul în zile peste data stadiului', () => {
      const rezultate = matchLiniiCuStadii([linii[0]!], [stadii[0]!], undefined, 3)
  ...
  describe('detectDuplicates', () => {
    const liniiCuData: LinieCuData[] = [
      { ...linii[0]!, dataPlanificata: '2026-04-10' },
      { ...linii[1]!, dataPlanificata: '2026-05-01' },
    ]
