...
- Direct campaign writes are not granted to `anon` or `authenticated`. Public users may read active campaign meters and milestones, while tenant users may read their own campaign data; atomic count and reward mutations must go through the dedicated `SECURITY DEFINER` RPC layer introduced separately.
