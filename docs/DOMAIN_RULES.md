...
- Direct campaign writes are not granted to `anon` or `authenticated`. Public users read only active campaign meter fields and milestones through `GET /api/shop/campaign/[slug]`; no customer or order data is exposed.
