...
- `shop_orders` delivery routing is day-scoped by nullable `delivery_date`, with legacy fallback to the Bucharest calendar date of `created_at`; route order is stored in nullable `delivery_position`. Public delivery checkout may send an optional preferred date from tomorrow through 60 days, while ERP operators may set or clear that date inline for non-cancelled preorders.
- `shop_orders` reads and updates are tenant-scoped through `tenant_id = public.current_tenant_id()`. The public order API assigns `tenant_id` exclusively from the required server-side `SHOP_TENANT_ID`; generic service-role PATCH operations must also filter by both order ID and session tenant and treat zero affected rows as not found.
- Seasonal shop campaigns are tenant-owned through `shop_campaigns.tenant_id -> public.tenants.id`. Milestones, manual count adjustments, and milestone rewards belong to a campaign; a milestone reward is unique per `(campaign_id, milestone_id)`, and a reached milestone remains reached even if its originating order is later cancelled. Existing shop orders default to `order_kind = 'standard'`; `campaign_id` is nullable, so standard orders remain independent from campaigns.
- Direct campaign writes are not granted to `anon` or `authenticated`. Public users read active campaign meter fields, milestones, and an anonymized top-10 leaderboard through `GET /api/shop/campaign/[slug]`; the response may expose only `anonId`, nullable `city`, aggregate package count, and a nullable season-prize label.
- `campaign_leaderboard` groups non-cancelled preorder quantities by campaign and customer phone. Its public `anon_id` contract now contains a masked label derived from the first token of the latest order name (`***` for tokens up to three characters, otherwise `***` plus the final three characters), never the full name or phone. The view is queried only with service role by the campaign API, has no `anon` grant, and must never expose customer phone, full name, or delivery address.
- Campaign administration keeps two deliberately separate counters: `shop_campaigns.current_count` remains the milestone/progress counter, while `/comenzi/campanie` derives active order totals from non-cancelled `shop_orders`. Daily, delivery-mode, and `in_suceava` breakdowns use the same active set; status counts use a separate tenant- and campaign-scoped query that also includes cancelled orders. The same active set drives a seven-day delivery schedule (Bucharest today plus six days), with undated orders reported separately.
- A campaign preorder must use `place_preorder_atomic`, never a direct `shop_orders` insert followed by separate campaign updates. The service-role-only RPC serializes writes per campaign with an advisory transaction lock and `FOR UPDATE`, inserts the order as `order_kind = 'preorder'`, increments `current_count` by the validated item quantity, and marks/rewards the first unreached milestone crossed by that specific order in the same transaction. Public checkout sends one stable `p_idempotency_key` per checkout session and regenerates it only after success/reset; delivery sends `p_in_suceava`, `deliveryZone`, and optional `p_preferred_delivery_date`, but the checkout UI no longer enforces a minimum package count by zone. For compatibility with older database functions that still reject on the legacy minimum rule, `POST /api/shop/b2c/order` retries the same RPC once with `p_in_suceava = null`; locality and `deliveryZone` still remain part of the order payload and ERP reporting.
...
- In Tratamente V2 manual intervention flows, manual product names are auto-saved best-effort in tenant library (`produse_fitosanitare`) only after intervention save succeeds, with fuzzy dedup (`Fuse` threshold `0.3`) and a non-blocking user choice (`asociază` vs `creează nou`).
