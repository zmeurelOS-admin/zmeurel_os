...
- `campaign_leaderboard` groups non-cancelled preorder quantities by campaign and customer phone, but exposes the phone only as a stable uppercase eight-character SHA-256 prefix. The view is queried only with service role by the campaign API, has no `anon` grant, and must never expose customer phone, name, or delivery address.
