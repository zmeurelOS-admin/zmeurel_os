...
- Există acum un proiect Supabase separat pentru staging (`zmeurelOS-staging` / `qinpqsqeaagjfobqwfwx`) și un preview Vercel `READY` pentru branch-ul `staging` la `https://zmeurel-git-staging-zmeurelos-admins-projects.vercel.app`, cu env-urile minime de drill confirmate.
- Riscul operațional rămas pentru staging este procedural: repo-ul principal rămâne linked la dev, iar targeting-ul către staging necesită în continuare un worktree separat + `supabase link` targetat până când există o soluție repo-level mai explicită decât lipsa actuală a `supabase/config.toml`.
